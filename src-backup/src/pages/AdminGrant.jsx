...
export default function AdminGrant() {
  ...
  // ✅ دالة آمنة لجلب الرصيد مع التحقق من UUID
  const fetchUserBalance = async (userId) => {
    // التحقق من أن userId هو UUID صالح
    if (!userId || userId === "" || userId === "all") {
      console.warn("Invalid userId for balance fetch:", userId);
      setCurrentBalance(0);
      return;
    }
  ...
  const handleGrantCoins = async () => {
    ...
    try {
      // ✅ استخدام RPC function بدلاً من direct insert (أكثر أماناً)
      const { data, error } = await supabase
        .rpc("grant_coins", {
